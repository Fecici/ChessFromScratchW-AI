"""
Ideas:
- for each move, make a grid copy and make a list of pieces based on that copy. check if the king is in check using the classes in the list. remove moves from move list and if list is empty, king is in checkmate
- redo mouse movement and checks

"""

import pygame; import sys; from Chess.previous_failed_versions.pieces_ver_3 import King, Queen, Pawn, Bishop, Knight, Rook
...
class Game:
    ...
    def create_grid(self, grid):
        ...
        for i, row in enumerate(grid): # TESTING GRIDS HERE
            ...
            for j, col in enumerate(row):
                ...
                piece = grid[i][j]
                if piece == 'br':
                    square.piece = Rook(self, j, i, piece[0], piece) # game class, x, y, colour, id
                    black_pieces.append(square.piece)

                if piece == 'bn':
                    square.piece = Knight(self, j, i, piece[0], piece)
                    black_pieces.append(square.piece)

                if piece == 'bb':
                    square.piece = Bishop(self, j, i, piece[0], piece)
                    black_pieces.append(square.piece)

                if piece == 'bq':
                    square.piece = Queen(self, j, i, piece[0], piece)
                    black_pieces.append(square.piece)

                if piece == 'bk':
                    square.piece = King(self, j, i, piece[0], piece)
                    black_pieces.append(square.piece)

                if piece == 'bp':
                    square.piece = Pawn(self, j, i, piece[0], piece)
                    black_pieces.append(square.piece)
                ...
                if piece == 'wr':
                    square.piece = Rook(self, j, i, piece[0], piece)
                    white_pieces.append(square.piece)
        ...
        return black_pieces, white_pieces, squares, grid
                ...
                for row in self.squares:
                    for square in row:
                        if square.rect.collidepoint(pos):
                            ...
                            print(self.highlighted_piece, selected_square)
                            if self.highlighted_piece == None and selected_square.piece != None:
                                self.highlighted_piece = selected_square
                                print(self.highlighted_piece, selected_square)

                            # the logic may be broken here   # no shit sherlock, need to go line by line and get a solid solution
                            # if selected_square != self.highlighted_piece:
                                for movable_squares in self.squares_to_move: # if i forget why this iter name is funny, just ask jodran (changede to movable_squares from lines_persisting)
                                    movable_squares.clicked = False
                                    movable_squares.legal_move = False
                                    # if self.highlighted_piece != None:
                                    #     self.highlighted_piece.clicked = False
                                    #     self.highlighted_piece.legal_move = False # redundant, highlighted_piece is already None
                                self.squares_to_move = []
                                self.highlighted_piece = selected_square # its good up to here I think

                            if selected_square == self.highlighted_piece: # if ive clicked the already selected square
                                selected_square.clicked = False
                                for movable_squares in self.squares_to_move: # if i forget why this iter name is funny, just ask jodran
                                    movable_squares.clicked = False
                                    movable_squares.legal_move = False
                                    if self.highlighted_piece != None:
                                        self.highlighted_piece.clicked = False
                                        self.highlighted_piece.legal_move = False
                                self.squares_to_move = []
                                self.highlighted_piece = None


                            if self.highlighted_piece.piece != None:
                                self.squares_to_move = self.check_legal_moves(self.highlighted_piece)
    ...
    def start(self):
        self.black_pieces, self.white_pieces, self.squares, self.GRID = self.create_grid(self.testing_grid)
