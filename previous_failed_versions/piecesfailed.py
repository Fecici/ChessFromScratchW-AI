...
class Pawn:
    ...
    def valid_moves(self, absolute=False, control=False): # need to add en passant and promotions
        legal_moves = []
        ...
        if self.colour == 'w':
            ...
            if j > 0 and square[j - 1][i].piece == None and not absolute: # and king is not in check
                legal_moves.append(square[j - 1][i])
                if j == 6 and square[j - 2][i].piece == None and not absolute:
                    legal_moves.append(square[j - 2][i])

            # capturing
            if (j > 0 and i > 0 and square[j - 1][i - 1].piece != None and square[j - 1][i - 1].piece.colour == 'b') or (j > 0 and i > 0 and (absolute or control)):
                legal_moves.append(square[j - 1][i - 1])

            if (j > 0 and i < 7 and square[j - 1][i + 1].piece != None and square[j - 1][i + 1].piece.colour == 'b') or (j > 0 and i < 7 and (absolute or control)):
                legal_moves.append(square[j - 1][i + 1])

        elif self.colour == 'b':

            if j < 7 and square[j + 1][i].piece == None and not absolute: # and king is not in check
                legal_moves.append(square[j + 1][i])
                if j == 1 and square[j + 2][i].piece == None and not absolute:
                    legal_moves.append(square[j + 2][i])

            if (j < 7 and i > 0 and square[j + 1][i - 1].piece != None and square[j + 1][i - 1].piece.colour == 'w') or (j < 7 and i > 0 and (absolute or control)):
                legal_moves.append(square[j + 1][i - 1])

            if (j < 7 and i < 7 and square[j + 1][i + 1].piece != None and square[j + 1][i + 1].piece.colour == 'w') or (j < 7 and i < 7 and (absolute or control)):
                legal_moves.append(square[j + 1][i + 1])
...
class King:
    ...
    def in_check(self): # take all enemy squares and check that the desired move is not in them or that king is not in check during its move
        ...
        if self.colour == 'w':
            print('w')
            print(self.game.black_pieces, 'hererr')
            for i in self.game.black_pieces:
                bad_squares = i.valid_moves(absolute=True)
                print(f'{bad_squares} IN CHECK')
            ...
            print('b')
            print(self.game.white_pieces, 'hererr')
    ...
    def controlled(self, s):
        controlled_squares = []

        if self.colour == 'w':
            for i in self.game.black_pieces:
                if i.class_value != 'king':
                    moves = i.valid_moves(control=True)
                else:
                    moves = i.valid_moves(absolute=True, control=True)

                for move in moves:
                    controlled_squares.append(move)
        ...
        if s in controlled_squares:
            return True

        return False

    def valid_moves(self, absolute=False, control=False): # how the fuck does this entire method even work properly??
        legal_moves = []
        ...
        if i > 0:
            ...
            if s.piece == None:
                ...
                if (s.piece.colour != self.colour and not self.controlled(s)) or control:
        ...
        print(f'{self.colour, legal_moves}\n')
...
class Queen:
    ...
    def valid_moves(self, absolute=False, control=False): # copy and pasted from rook and bishop classes
        legal_moves = []
        ...
        for iter in range(1, nw):
            ...
            if s.piece == None:
                ...
                if absolute and s.piece.class_value == 'king' and s.piece.colour != self.colour:
                    legal_moves.append(s)
                else:
                    if s.piece.colour != self.colour or control:
                        legal_moves.append(s)
                    break
...
class Rook:
    ...
    def valid_moves(self, absolute=False, control=False):
        legal_moves = []
...
class Knight:
    ...
    def valid_moves(self, absolute=False, control=False): # absolute is unused but kept for iteration purposes in King().in_check()
        legal_moves = []
