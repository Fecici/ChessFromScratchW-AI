class Pawn:
    ...
    def __init__(self, game, x, y, colour, piece_id, grid):
        self.id = piece_id
        self.game = game
        self.colour = colour
    ...
    def valid_moves(self, squares_array, cull_illegal_moves=False): # need to add en passant and promotions
        ...
        if cull_illegal_moves:
            bad_moves = self.game.illegal_moves(self.grid, legal_moves, square[j][i])
    ...
    def __repr__(self):
        return f'[{self.class_value}; y, x: {(self.y//self.game.Ytl, self.x//self.game.Xtl)}]'
...
class King:
    ...
    def valid_moves(self, squares_array, cull_illegal_moves=False, add_castling=False):
        ...
        if add_castling:
            if self.game.check_castling(square, self.game.white_pieces, self.game.black_pieces, self.game.white_moves_unculled, self.game.black_moves_unculled, longue=True):
                legal_moves.append(square[j][i - 2])

            if self.game.check_castling(square, self.game.white_pieces, self.game.black_pieces, self.game.white_moves_unculled, self.game.black_moves_unculled, short=True):
                legal_moves.append(square[j][i + 2])
